  ...
  return (
    <div className="flex items-stretch gap-3 md:gap-4 overflow-x-auto scrollbar-hide scroll-smooth pb-2 snap-x snap-mandatory">
      {filteredJobs.map((job) => (
        <article
          key={job.id}
          className="w-40 md:w-44 shrink-0 rounded-lg overflow-hidden bg-white/10 hover:bg-white/15 transition cursor-pointer relative group snap-start"
        >
          <Link href={`/jobs/${job.id}`}>
            {/* Image/Logo de l'entreprise */}
            <div className="h-20 md:h-24 w-full overflow-hidden bg-white/20">
              {job.companyLogo ? (
                <img src={job.companyLogo} alt={job.company} className="h-full w-full object-cover" />
              ) : (
                <div className="h-full w-full flex items-center justify-center">
                  <Briefcase className="w-10 h-10 md:w-12 md:h-12 text-white/70" />
                </div>
              )}
            </div>
            ...
            <div className="px-2.5 md:px-3 py-2">
              <p className="text-[10px] md:text-[11px] leading-snug line-clamp-2">{job.title}</p>
            </div>
          </Link>
          ...
          <button
            ...
            className={`absolute top-2 right-2 p-1.5 rounded-full transition-all backdrop-blur ${
